...
SELECT
  ...
  SAFE_CAST(SPLIT(JSON_VALUE(PAYLOAD, '$.response.afgeronddatum'), '.')[OFFSET(0)] AS DATETIME) AS TRAJECT_AFGERONDDATUM,
  ...
  SAFE_CAST(SPLIT(JSON_VALUE(PAYLOAD, '$.response.creatiedatum'), '.')[OFFSET(0)] AS DATETIME) AS TRAJECT_CREATIEDATUM,
...
AND JSON_VALUE(PAYLOAD, '$.SOORTTRAJECTID') = "2" 
AND JSON_VALUE(PAYLOAD, '$.TRAJECTSTATUSID') IN ("1", "2", "3")
AND JSON_VALUE(PAYLOAD, '$.DEFINITIEFOFFERTEID') <> ""
`
let refs = pk.getRefs()
