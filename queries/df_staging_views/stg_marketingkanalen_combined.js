...
SELECT  
   ...
   ${ifNull([
        ...
        ifSource("gs_merken", `${ref("lookup_table_sql")}(ads_merk_concat, lookup_merken.haystack)`)
    ], "as merk,")}
    ${ifNull([
        orSource(['googleads_campaignlabel', 'stg_bing_ad_group_performance'], 'model'),
        ifSource('gs_modellen', `${ref("lookup_table_sql")}(ads_merk_concat, lookup_modellen.haystack)`)
    ], "as model,")}
FROM (
   SELECT 
    *,
