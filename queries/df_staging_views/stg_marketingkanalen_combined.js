...
SELECT 
   ...
   ${ifNull([
    ...
    ${ifNull([
        ...
        ifSource('gs_modellen', `${ref("lookupTable")}(campaign_name, INITCAP(TO_JSON_STRING(ARRAY(SELECT model FROM ${ref("df_googlesheets_tables", "gs_modellen", true)}))))`),
        ifSource('gs_modellen', `${ref("lookupTable")}(ad_group_name, INITCAP(TO_JSON_STRING(ARRAY(SELECT model FROM ${ref("df_googlesheets_tables", "gs_modellen", true)}))))`)
    ], "as model,")}
FROM (
    SELECT
        ...
        ${ifNull(['google_ads.segments_date', ifSource('stg_facebookdata','facebook.date_start'), ifSource('dv360_data','dv360.date'), ifSource('stg_bing_ad_group_performance','CAST(microsoft.time_period AS DATE)'), ifSource('stg_linkedin_ads_combined','linkedin.startDate'), ifSource('stg_handmatige_uitgaves_pivot', 'handmatig.record_datum')])} as record_date,
        ...
        ${ifNull(['google_ads.Cost', ifSource('stg_facebookdata','facebook.spend'), ifSource('dv360_data','dv360.revenue_advertiser_currency'), ifSource('stg_bing_ad_group_performance','microsoft.spend'), ifSource('stg_linkedin_ads_combined','linkedin.cost'), ifSource('stg_handmatige_uitgaves_pivot', 'handmatig.uitgaven')])} as ads_cost,
