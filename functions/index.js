...
function ref(p1, p2) {
    ...
    for(let s in sources) {
        if( //if the ref has only one parameter it has to be the name, when there are 2 parameter the second wil be the name. (name is interchangable with alias)
            (typeof p2 == "undefined" && (sources[s].name === p1 || (sources[s].alias === p1 && typeof sources[s].name == 'undefined') ) )
            ||
            (typeof p2 != "undefined" && (sources[s].name === p2 || (sources[s].alias === p2 && typeof sources[s].name == 'undefined') ) && sources[s].schema === p1)
        ){
