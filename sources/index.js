...
function addSuffix(schema) {
    if (!schema.startsWith("ads_") && !schema.startsWith("analytics_") && schema !== "rawdata" && schema !== "googleSheets" && dataform.projectConfig.schemaSuffix !== "") {
        return schema+"_"+dataform.projectConfig.schemaSuffix ?? "WRONG"
    }
    return schema
}
...
function ref(p1, p2, ifSource) {
    ...
    for(let s in sources) {
        if( //if the ref has only one parameter it has to be the name, when there are 2 parameter the second wil be the name. (name is interchangable with alias)
            ...
            if(!(sources[s].noSuffix ?? false) && !sources[s].schema.startsWith("analytics_") && sources[s].schema !== "rawdata" && sources[s].schema !== "googleSheets" && dataform.projectConfig.schemaSuffix !== "") { r.query += "_" + dataform.projectConfig.schemaSuffix ?? "STILL_WRONG" }
...
function schemaSuffix(source) {
    if(source.schema !== "rawdata" && source.schema !== "googleSheets" && dataform.projectConfig.schemaSuffix !== "") { return "_" + dataform.projectConfig.schemaSuffix ?? "ALSO_WRONG" } else {return ""}
}
